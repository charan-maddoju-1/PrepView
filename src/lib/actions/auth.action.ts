...
export async function setSessionCookie(idToken:string){
    console.log(">>> setSessionCookie() called on server");
...
export async function getCurrentUser(): Promise <User|null>{
    ...
    console.log("Session cookie value on server:", sessionCookie ? "exists" : "missing");
...
export async function isAuthenticated(){
    ...
    console.log(user);
    return !!user; //this give true if there exists a user else false
}
...
export async function getLatestInterviews(params:GetLatestInterviewsParams): Promise<Interview[]>{
    ...
    const interviews=await db.collection('interviews').where('userId','==',userId).orderBy('createdAt','desc').limit(limit).get();
    return interviews.docs.map((doc)=>({
        id:doc.id,
        ...doc.data()
    })) as Interview[];
